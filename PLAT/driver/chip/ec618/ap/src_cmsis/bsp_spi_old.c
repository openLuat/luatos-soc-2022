...
int32_t SPI_ReadFIFO(const void *data, SPI_RESOURCES *spi)
{
    uint8_t len;
    uint8_t *rx_buf;
    ...
    len = 0;
    rx_buf = (uint8_t *)data;
    while(((spi->reg->SR & SPI_SR_RNE_Msk) != 0) && len < SPI_RX_FIFO_TRIG_LVL)
    {
        *rx_buf++ = (uint8_t)spi->reg->DR;
        len++;
    }
...
int32_t SPI_Transfer(const void *data_out, void *data_in, uint32_t num, SPI_RESOURCES *spi)
{
    ...
    //uint32_t before = osKernelGetTickCount();
    //while (spi->reg->SR & SPI_SR_BSY_Msk);
    ...
    //    spi->reg->DR;
    //}
    //uint32_t after  = osKernelGetTickCount();
    ...
    if(spi->dma)
    {
        ...
        g_dmaRxConfig.addressIncrement = DMA_ADDRESS_INCREMENT_TARGET;
        g_dmaRxConfig.sourceAddress = (void *)&(spi->reg->DR);
        g_dmaRxConfig.targetAddress = (void *)&((char *)data_in)[SPI_RX_FIFO_TRIG_LVL];
        g_dmaRxConfig.totalLength   = num * data_width;
...
int32_t SPI_Control(uint32_t control, uint32_t arg, SPI_RESOURCES *spi)
{
    ...
    if((control & ARM_SPI_CONTROL_Msk) == ARM_SPI_ABORT_TRANSFER)
    {
        ...
        if(spi->info->status.busy)
        {
            ...
            if(spi->dma)
            {
                ret = DMA_stopChannel(spi->dma->tx_instance, spi->dma->tx_ch, true);
                ret = DMA_stopChannel(spi->dma->rx_instance, spi->dma->rx_ch, true);
            }
        }
...
static int32_t SPI0_ReadFIFO(const void *data)
{
    return SPI_ReadFIFO(data, &SPI0_Resources);
}
void SPI0_IRQHandler(void)
{
...
static int32_t SPI1_ReadFIFO(const void *data)
{
    return SPI_ReadFIFO(data, &SPI1_Resources);
}
void SPI1_IRQHandler(void)
{

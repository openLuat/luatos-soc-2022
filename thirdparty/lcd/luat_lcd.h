...
int luat_lcd_close(luat_lcd_conf_t* conf);//关闭lcd显示屏
int luat_lcd_display_on(luat_lcd_conf_t* conf);//开启lcd显示屏背光
int luat_lcd_display_off(luat_lcd_conf_t* conf);//关闭lcd显示屏背光
int luat_lcd_sleep(luat_lcd_conf_t* conf);//lcd睡眠
int luat_lcd_wakeup(luat_lcd_conf_t* conf);//lcd唤醒
int luat_lcd_inv_off(luat_lcd_conf_t* conf);//lcd反显关闭
int luat_lcd_inv_on(luat_lcd_conf_t* conf);//lcd反显
int luat_lcd_set_address(luat_lcd_conf_t* conf,uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2);
int luat_lcd_set_color(luat_color_t back, luat_color_t fore);//lcd颜色设置
luat_color_t color_swap(luat_color_t color);
int luat_lcd_draw(luat_lcd_conf_t* conf, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, luat_color_t* color);//lcd颜色填充
int luat_lcd_flush(luat_lcd_conf_t* conf);//主动刷新数据到界面, 仅设置buff且禁用自动属性后使用 本API与 lcd.setupBuff lcd.autoFlush 配合使用
...
int luat_lcd_clear(luat_lcd_conf_t* conf,luat_color_t color);//lcd清屏
int luat_lcd_draw_fill(luat_lcd_conf_t* conf,uint16_t x1,uint16_t y1,uint16_t x2,uint16_t y2,luat_color_t color);//lcd颜色填充
int luat_lcd_draw_point(luat_lcd_conf_t* conf, uint16_t x, uint16_t y, luat_color_t color);//画一个点.
int luat_lcd_draw_line(luat_lcd_conf_t* conf,uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, luat_color_t color);//在两点之间画一条线

...
static int32_t luat_spitf_cmd(luat_spitf_ctrl_t *spitf, uint8_t Cmd, uint32_t Arg, uint8_t NeedStop)
{
	...
	if (Result)
	{
		...
		luat_debug_dump(spitf->TempData, TxLen);
	}
	return Result;
}
...
static void luat_spitf_init(luat_spitf_ctrl_t *spitf)
{
	...
	if (!spitf->Info)
	{
		spitf->Info = malloc(sizeof(SD_CardInfo));
	}
	memset(spitf->Info, 0, sizeof(SD_CardInfo));
	if (!spitf->TempData)
	{
		spitf->TempData = malloc(__SDHC_BLOCK_LEN__ + 8);
	}
	...
	memset(spitf->TempData, 0xff, 80);
	luat_gpio_set(spitf->CSPin, 0);
	luat_spi_transfer(spitf->SpiID, (const char *)spitf->TempData, 40, (char *)spitf->TempData, 40);
	luat_gpio_set(spitf->CSPin, 1);
	memset(spitf->TempData, 0xff, 80);
	luat_spi_transfer(spitf->SpiID, (const char *)spitf->TempData, 80, (char *)spitf->TempData, 80);
	spitf->SDSC = 0;
	...
	if (luat_spitf_cmd(spitf, CMD8, 0x1aa, 1))
	{
